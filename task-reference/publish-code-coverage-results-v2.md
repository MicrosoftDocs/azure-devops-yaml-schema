...
Unlike the  [PublishCodeCoverageResults@1](publish-code-coverage-results-v1.md) task, v2 can generate the [diff](/azure/devops/pipelines/test/codecoverage-for-pullrequests#full-coverage-diff-coverage) coverage for multiple languages in the same pipeline. Additionally, the code coverage tab of the PublishCodeCoverageResultsV2 task is the same like the one from the PublishCodeCoverageV1 task.
...
Prerequisite- To use the Publish Code Coverage Results v2 task in the pipeline, please use the [Use .NET Core](./use-dotnet-v2.md) task as a pre-requisite in the pipeline. Use the dotnet core task before the Publish Code Coverage v2 task. 
...
2. After the build completes and the Publish Code Coverage Results v2 task succeeds, select the **Code Coverage** tab in the pipeline run summary to view the code coverage results.
   ...
   :::image type="content" source="media/code-coverage-results.png" alt-text="Screenshot that shows the Code Coverage tab generated by the Publish Code coverage V2 task":::
